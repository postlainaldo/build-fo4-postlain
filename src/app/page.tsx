...
const FORMATIONS: Record<FormationName, { role: string; top: string; left: string }[]> = {
  '4-2-3-1': [
    ...
    { role: 'LB', top: '70%', left: '12%' },
    { role: 'LCB', top: '73%', left: '35%' },
    { role: 'RCB', top: '73%', left: '65%' },
    { role: 'RB', top: '70%', left: '88%' },
    { role: 'LDM', top: '53%', left: '33%' },
    { role: 'RDM', top: '53%', left: '67%' },
    { role: 'LAM', top: '30%', left: '16%' },
    { role: 'CAM', top: '26%', left: '50%' },
    { role: 'RAM', top: '30%', left: '84%' },
  ...
  '4-3-3': [
    ...
    { role: 'RCB', top: '73%', left: '65%' },
    { role: 'RB', top: '70%', left: '88%' },
    { role: 'LCM', top: '48%', left: '25%' },
    { role: 'CM', top: '53%', left: '50%' },
    { role: 'RCM', top: '48%', left: '75%' },
    { role: 'LW', top: '20%', left: '16%' },
    { role: 'ST', top: '10%', left: '50%' },
    { role: 'RW', top: '20%', left: '84%' },
  ],
  '4-1-2-3': [
    { role: 'GK', top: '88%', left: '50%' },
    { role: 'LB', top: '70%', left: '12%' },
    ...
    { role: 'RCB', top: '73%', left: '65%' },
    { role: 'RB', top: '70%', left: '88%' },
    { role: 'CDM', top: '55%', left: '50%' },
    { role: 'LCM', top: '42%', left: '28%' },
    { role: 'RCM', top: '42%', left: '72%' },
    { role: 'LW', top: '18%', left: '16%' },
    { role: 'ST', top: '10%', left: '50%' },
    { role: 'RW', top: '18%', left: '84%' },
  ],
  '5-2-3': [
    { role: 'GK', top: '88%', left: '50%' },
    { role: 'LWB', top: '65%', left: '10%' },
    { role: 'LCB', top: '73%', left: '28%' },
    { role: 'CB', top: '74%', left: '50%' },
    { role: 'RCB', top: '73%', left: '72%' },
    { role: 'RWB', top: '65%', left: '90%' },
    { role: 'LCM', top: '45%', left: '33%' },
    { role: 'RCM', top: '45%', left: '67%' },
    { role: 'LW', top: '18%', left: '16%' },
    { role: 'ST', top: '10%', left: '50%' },
    { role: 'RW', top: '18%', left: '84%' },
  ],
  '3-5-2': [
    { role: 'GK', top: '88%', left: '50%' },
    { role: 'LCB', top: '73%', left: '25%' },
    { role: 'CB', top: '74%', left: '50%' },
    { role: 'RCB', top: '73%', left: '75%' },
    { role: 'LDM', top: '53%', left: '35%' },
    { role: 'RDM', top: '53%', left: '65%' },
    { role: 'LM', top: '38%', left: '12%' },
    { role: 'CAM', top: '32%', left: '50%' },
    { role: 'RM', top: '38%', left: '88%' },
    { role: 'LS', top: '12%', left: '33%' },
    { role: 'RS', top: '12%', left: '67%' },
  ]
};
...
function SquadBuilderContent() {
  ...
  useEffect(() => {
    ...
    if (shareToken) {
      ...
      if (decoded) {
        if (decoded.formation in FORMATIONS) setFormation(decoded.formation as FormationName);
        if (decoded.cap) setSalaryCap(decoded.cap);
  ...
  const handleSelectPlayer = (player: Player) => {
    if (activeSlotIndex !== null) {
      setSelectedPlayers(prev => ({ ...prev, [activeSlotIndex]: player }));
    }
  ...
  const handleRemovePlayer = (index: number, e: React.MouseEvent) => {
    e.stopPropagation();
    setSelectedPlayers(prev => ({ ...prev, [index]: null }));
  };

  const handleClearSquad = () => {
    if (confirm('Bạn có chắc chắn muốn xoá toàn bộ đội hình hiện tại không?')) {
      setSelectedPlayers({ 0: null, 1: null, 2: null, 3: null, 4: null, 5: null, 6: null, 7: null, 8: null, 9: null, 10: null });
    ...
    <div className="flex-1 w-full max-w-7xl mx-auto px-4 py-6 space-y-6">
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
              ...
              <div className="flex bg-[#0B0F19] rounded-xl p-1 border border-gray-700/80">
                {SALARY_CAPS.map(cap => (
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-lg text-xs font-bold cursor-pointer min-h-[36px] transition-all ${salaryCap === cap ? 'bg-fo4-accent text-fo4-dark shadow-md' : 'text-gray-400 hover:text-white'}`}
                  >
                    {cap}
                  </button>
                ))}
        ...
        <div className="lg:col-span-5 xl:col-span-4 flex flex-col space-y-6">
          <div className="bg-fo4-card border border-gray-800 rounded-2xl p-5 shadow-2xl relative overflow-hidden">
            ...
            <div className="grid grid-cols-3 gap-2 sm:gap-3.5">
              <div className="bg-[#0B0F19] border border-gray-800 rounded-xl p-2.5 sm:p-3 text-center flex flex-col justify-center">
                <span className="text-gray-400 text-[8px] sm:text-[10px] font-extrabold uppercase tracking-wider block mb-1">OVR ĐỘI HÌNH</span>
              ...
              <div className={`border rounded-xl p-2.5 sm:p-3 text-center flex flex-col justify-center transition-all ${isOverSalary ? 'bg-red-950/20 border-red-700/50' : 'bg-[#0B0F19] border-gray-800'}`}>
                <span className="text-gray-400 text-[8px] sm:text-[10px] font-extrabold uppercase tracking-wider block mb-1">LƯƠNG ĐÃ DÙNG</span>
                <div className="flex items-center justify-center space-x-1">
                  <Coins className={`w-3.5 h-3.5 sm:w-4 sm:h-4 ${isOverSalary ? 'text-red-500' : 'text-fo4-accent'}`} />
                  <span className={`text-sm sm:text-lg font-black ${isOverSalary ? 'text-red-500' : 'text-white'}`}>{totalSalary}</span>
                  <span className="text-[10px] text-gray-500">/{salaryCap}</span>
                </div>
              </div>

              <div className="bg-[#0B0F19] border border-gray-800 rounded-xl p-2.5 sm:p-3 text-center flex flex-col justify-center">
                <span className="text-gray-400 text-[8px] sm:text-[10px] font-extrabold uppercase tracking-wider block mb-1">GIÁ TRỊ ĐỘI</span>
                <div className="flex items-center justify-center space-x-1">
                  <TrendingUp className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-[#FFD700]" />
                  <span className="text-sm sm:text-lg font-black text-[#FFD700]">{totalValue.toLocaleString('vi-VN')}</span>
            ...
            {isOverSalary && (
              <div className="mt-3 flex items-center space-x-2 bg-red-950/40 border border-red-700/40 p-3 rounded-xl text-red-400 animate-pulse">
                ...
                <p className="text-[11px] font-bold leading-normal">Đội hình vượt quá quỹ lương ({salaryCap} FP)! Hãy giảm lương cầu thủ để tham gia đá xếp hạng.</p>
              </div>
            )}
          </div>
          ...
          <ShareSavePanel formation={formation} selectedPlayers={selectedPlayers} currentCap={salaryCap} onClearSquad={handleClearSquad} />
        </div>
  ...
  return (
    <>
      ...
      <main className="flex-1">
        <Suspense fallback={<div className="flex items-center justify-center py-20 text-gray-400 text-sm">Đang tải kho dữ liệu cầu thủ...</div>}><SquadBuilderContent /></Suspense>
      </main>
