...
const GET_SEASON_CARD_STYLE = (season: string) => {
  ...
  return { bg: 'bg-gradient-to-b from-[#374151] to-[#111827]', border: 'border border-gray-600', text: 'text-white', numText: 'text-gray-300', badgeBg: 'bg-gray-700 text-white' };
};
...
export default function PlayerSelectorModal({ isOpen, onClose, onSelectPlayer, targetRole }: PlayerSelectorModalProps) {
  ...
  const filteredPlayers = useMemo(() => {
    return PLAYERS_DB.filter(p => {
      const mSearch = p.name.toLowerCase().includes(searchTerm.toLowerCase()) || p.club.toLowerCase().includes(searchTerm.toLowerCase()) || p.nationality.toLowerCase().includes(searchTerm.toLowerCase());
        ...
        <div className="p-3.5 border-b border-gray-800 bg-[#111A2C] space-y-3 shrink-0">
          <div className="flex items-center space-x-2">
            <div className="relative flex-1">
              ...
              <input type="text" placeholder="Tìm tên siêu sao, ví dụ: Ronaldo, Messi, Quang Hải..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full bg-[#182335] text-white pl-11 pr-4 py-2.5 rounded-xl border border-gray-700/60 focus:border-fo4-accent outline-none text-sm transition" />
            </div>
            <button onClick={() => setShowFilters(!showAdvancedFilters)} className={`flex items-center space-x-1.5 px-3 py-2.5 rounded-xl border text-xs font-bold transition cursor-pointer shrink-0 ${showAdvancedFilters || selectedSeason !== 'Tất cả' || selectedPosition !== 'Tất cả' ? 'bg-fo4-accent/15 border-fo4-accent text-fo4-accent' : 'bg-[#182335] border-gray-700 text-gray-300 hover:text-white'}`}><Sliders className="w-4 h-4" /><span className="hidden sm:inline">⚙️ BỘ LỌC</span></button>
          </div>
          ...
          {showAdvancedFilters && (
            <div className="p-3.5 bg-[#0B1220] rounded-xl border border-gray-800 space-y-4 animate-in slide-in-from-top-3 duration-200">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-1.5 border-t border-gray-800/60">
                <div>
                  <div className="flex justify-between text-[10px] text-gray-400 mb-1"><span>Lương cầu thủ tối đa</span><span className="text-fo4-accent font-bold">{maxSalary} FP</span></div>
                ...
                <div>
                  <div className="flex justify-between text-[10px] text-gray-400 mb-1"><span>Chỉ số OVR tối thiểu</span><span className="text-fo4-gold font-bold">{minOVR} OVR</span></div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2.5 bg-[#0D1322]">
          ...
          {filteredPlayers.length > 0 ? (
            filteredPlayers.map(player => {
              ...
              return (
                <div key={player.id} onClick={() => onSelectPlayer(player)} className="flex items-center justify-between p-3.5 bg-fo4-card/65 hover:bg-fo4-card border border-gray-800/80 hover:border-fo4-accent/40 rounded-2xl transition cursor-pointer active:scale-[0.99] group shadow-sm animate-in fade-in duration-200">
                  ...
                  <div className="flex items-center space-x-3 sm:space-x-5">
                    ...
                    <div className="text-right border-l border-gray-800 pl-3.5 sm:pl-5 min-w-[50px] hidden sm:block"><span className="block text-[#FFD700] font-black text-sm sm:text-base">{player.value} Tỷ</span><span className="text-[9px] text-gray-500 uppercase font-bold block">BP</span></div>
                  </div>
                </div>
              );
            })
          ) : (
            <div className="text-center py-14 text-gray-500 bg-fo4-card/25 rounded-2xl border border-dashed border-gray-800"><p className="font-black text-sm text-gray-400">Không tìm thấy cầu thủ nào phù hợp</p><p className="text-xs mt-1 text-gray-500">Hãy thử giảm OVR tối thiểu hoặc kéo tăng quỹ lương tối đa!</p></div>
          )}
        </div>
      </div>
    </div>
